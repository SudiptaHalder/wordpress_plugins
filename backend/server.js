...
app.get("/api/seo", async (req, res) => {
  try {
    ...
    // 1. TOP LANDING PAGES (first page of each session)
    const [landingPages] = await pool.execute(`
      SELECT p.url, COUNT(*) AS visits
      FROM (
        SELECT session_id, MIN(viewed_at) AS first_view
        FROM pageviews
        GROUP BY session_id
      ) s
      JOIN pageviews p ON p.session_id = s.session_id AND p.viewed_at = s.first_view
      GROUP BY p.url
      ORDER BY visits DESC
      LIMIT 5
    `);

    // 2. TOP EXIT PAGES (last page of each session)
    const [exitPages] = await pool.execute(`
      SELECT p.url, COUNT(*) AS exits
      FROM (
        SELECT session_id, MAX(viewed_at) AS last_view
        FROM pageviews
        GROUP BY session_id
      ) s
      JOIN pageviews p ON p.session_id = s.session_id AND p.viewed_at = s.last_view
      GROUP BY p.url
      ORDER BY exits DESC
      LIMIT 5
    `);
    ...
    // 3. ORGANIC SEARCH
    const [organic] = await pool.execute(`
      SELECT COUNT(*) AS organic
      FROM traffic_sources
      WHERE source = 'google' OR medium = 'organic'
    `);

    // 4. SOCIAL TRAFFIC
    const [social] = await pool.execute(`
      SELECT COUNT(*) AS social
      FROM traffic_sources
      WHERE source IN ('facebook','twitter','instagram','linkedin')
    `);
    ...
    // 6. SEARCH KEYWORDS (if referrer contains q=)
    const [keywords] = await pool.execute(`
      SELECT 
        SUBSTRING_INDEX(SUBSTRING(referrer, LOCATE('q=', referrer) + 2), '&', 1) AS keyword,
        COUNT(*) as searches
      FROM analytics_events
      WHERE referrer LIKE '%q=%'
      GROUP BY keyword
      ORDER BY searches DESC
      LIMIT 10
    `);

    // 7. SEO SCORE (temp placeholder)
    const seoScore = Math.floor(60 + Math.random() * 30);

    res.json({
      seoScore,
      landingPages,
      exitPages,
      organicTraffic: organic[0].organic,
      socialTraffic: social[0].social,
      referralTraffic: referral[0].referral,
      keywords
    });

  } catch (err) {
    console.error("SEO API ERR:", err);
